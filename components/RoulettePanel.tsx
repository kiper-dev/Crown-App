...
import type { PageWidget, Profile, RouletteDraft, Social } from "@/lib/data/types";
...
const RL_WIDGET_LABEL: Record<PageWidget["kind"], string> = {
  donate: "Suggest form",
  socials: "Social icons",
};

// The Roulette page builder — deliberately the same experience as the Task and Fundraiser
...
export function RoulettePanel({ profile, onSave }: { profile: Profile; onSave: (p: Profile) => void }) {
  ...
  const link = `${origin || "https://crown.tv"}/@${profile.handle}/roulette`;
  ...
  // everything else lives on the roulette draft.
  function patchProfile(next: Partial<Profile>) {
    onSave({ ...profile, ...next });
  }

  function patchRl(next: Partial<RouletteDraft>) {
    onSave({ ...profile, roulette: { ...rl, ...next } });
  }

  function patchWidget(kind: PageWidget["kind"], next: Partial<PageWidget>) {
    patchRl({ widgets: rl.widgets.map((w) => (w.kind === kind ? { ...w, ...next } : w)) });
  }

  function moveWidget(index: number, dir: -1 | 1) {
    const to = index + dir;
    if (to < 0 || to >= rl.widgets.length) return;
    const next = rl.widgets.slice();
    [next[index], next[to]] = [next[to], next[index]];
    patchRl({ widgets: next });
  }
  ...
  function updatePreset(i: number, value: number) {
    const next = rl.presets.slice();
    next[i] = Math.max(1, Math.round(value) || 1);
    patchRl({ presets: next });
  }

  function addPreset() {
    const last = rl.presets[rl.presets.length - 1] ?? 5;
    patchRl({ presets: [...rl.presets, last + 5] });
  }

  function removePreset(i: number) {
    if (rl.presets.length <= 1) return;
    patchRl({ presets: rl.presets.filter((_, j) => j !== i) });
  }
  ...
  function onBackgroundFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => patchRl({ design: { background: { type: "image", value: String(reader.result) } } });
    reader.readAsDataURL(file);
  }
    ...
    <div className={styles.wrap}>
      <div className={styles.builder}>
        <div className={styles.editor}>
          <div className={styles.tabs} role="tablist" aria-label="Roulette builder">
                ...
                <div className={styles.bioField}>
                  ...
                  <div className={styles.bioBox}>
                    <textarea
                      ...
                      rows={2}
                      maxLength={RL_HEADLINE_MAX}
                      placeholder="e.g. You pick what I play next — suggest a game"
                      value={rl.headline}
                      onChange={(e) => patchRl({ headline: e.target.value })}
                    />
                    <span className={styles.charCount}>{rl.headline.length}/{RL_HEADLINE_MAX}</span>
                  </div>
                  <div className="footnote">The wheel picks by pool share — a suggestion that doesn't win stays donated either way.</div>
                </div>

                <div className={styles.bioField}>
                  <label className={`toggle${rl.descriptionEnabled ? " on" : ""}`}>
                    <span className="track"><span className="knob" /></span>
                    <input type="checkbox" hidden checked={rl.descriptionEnabled} onChange={(e) => patchRl({ descriptionEnabled: e.target.checked })} />
                    Description
                  </label>
                  <div className={styles.bioBox}>
                    <textarea
                      className={styles.bioInput}
                      rows={2}
                      maxLength={RL_DESCRIPTION_MAX}
                      placeholder="House rules: when the wheel spins, what's off the table, how long you play"
                      value={rl.description}
                      onChange={(e) => patchRl({ description: e.target.value })}
                    />
                    <span className={styles.charCount}>{rl.description.length}/{RL_DESCRIPTION_MAX}</span>
                  </div>
                </div>
              </div>
                    ...
                    <div key={w.kind} className={styles.widgetGroup}>
                      <div className={`${styles.widgetRow}${w.enabled ? "" : ` ${styles.widgetOff}`}`}>
                        ...
                        <div className={styles.widgetStepper}>
                          ...
                          <button type="button" aria-label="Move down" disabled={i === rl.widgets.length - 1} onClick={() => moveWidget(i, 1)}>▼</button>
                        </div>
                        <button type="button" className={styles.widgetNameBtn} aria-expanded={open} onClick={() => toggleWidgetConfig(w.kind)}>
                          {RL_WIDGET_LABEL[w.kind]}
                      ...
                      {open && w.kind === "donate" && (
                        <div className={styles.widgetConfig}>
                          <div className={styles.presetRow}>
                            ...
                            {rl.presets.length < MAX_RL_PRESETS && (
            ...
            <div className={styles.tabBody}>
              <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div className={styles.themeGrid}>
                  {THEMES.map((t) => {
                    const active = sameBackground(rl.design, t.design);
                    return (
                      <button
                        key={t.id}
                        type="button"
                        ...
                        onClick={() => patchRl({ design: t.design })}
                      >
              ...
              <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div className={styles.bgTypeRow}>
                  {(["color", "gradient", "image"] as const).map((t) => (
                    <button
                      key={t}
                      type="button"
                      className={`${styles.bgType} ${rl.design.background.type === t ? styles.bgTypeOn : ""}`}
                      onClick={() => {
                        if (t === "image") { bgFileRef.current?.click(); return; }
                        patchRl({ design: { background: { type: t, value: t === "color" ? BACKGROUND_COLOR_PRESETS[0] : BACKGROUND_GRADIENT_PRESETS[0].id } } });
                      }}
                    >
                ...
                {rl.design.background.type === "color" && (
                  <div className={styles.swatches}>
                    {BACKGROUND_COLOR_PRESETS.map((hex) => (
                      <button
                        ...
                        type="button"
                        aria-label={hex}
                        className={`${styles.swatch} ${rl.design.background.value === hex ? styles.swatchOn : ""}`}
                        style={{ background: hex }}
                        onClick={() => patchRl({ design: { background: { type: "color", value: hex } } })}
                      />
                    ))}
                    <input
                      className={styles.hexInput}
                      type="text"
                      value={rl.design.background.value}
                      onChange={(e) => patchRl({ design: { background: { type: "color", value: e.target.value } } })}
                    />
                  </div>
                )}

                {rl.design.background.type === "gradient" && (
                  <div className={styles.swatches}>
                    {BACKGROUND_GRADIENT_PRESETS.map((g) => (
                      <button
                        ...
                        type="button"
                        aria-label={g.label}
                        className={`${styles.swatch} ${rl.design.background.value === g.id ? styles.swatchOn : ""}`}
                        style={{ backgroundImage: g.css }}
                        onClick={() => patchRl({ design: { background: { type: "gradient", value: g.id } } })}
                      />
                    ))}
                  </div>
                )}

                {rl.design.background.type === "image" && (
                  <div className="footnote">
                    {rl.design.background.value ? "Image set — pick Choose file to replace it." : "No image chosen yet."}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className={styles.previewCol}>
          ...
          <LivePreview src={`/@${profile.handle}/roulette`} device={device} />

          <div className={styles.linkRow}>
            <div className={styles.linkLabel}>Roulette link</div>
            <a className={styles.linkChip} href={`/@${profile.handle}/roulette`} target="_blank" rel="noreferrer">
              ...
              <div className={styles.qrPop} role="dialog" aria-label="QR code">
                {qrDataUrl ? (
                  <>
                    ...
                    <a className={styles.qrDownload} href={qrDataUrl} download="crown-roulette-qr.png">
                      Download PNG
                    </a>
                  </>
                ) : (
