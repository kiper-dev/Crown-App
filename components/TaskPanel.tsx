...
import type { PageWidget, Profile, TaskDraft, Social } from "@/lib/data/types";
...
const TASK_WIDGET_LABEL: Record<PageWidget["kind"], string> = {
  donate: "Task form",
  socials: "Social icons",
};

// The Task page builder — deliberately the same experience as the Roulette and Fundraiser
...
export function TaskPanel({ profile, onSave }: { profile: Profile; onSave: (p: Profile) => void }) {
  ...
  const link = `${origin || "https://crown.tv"}/@${profile.handle}/task`;
  ...
  // everything else lives on the task-page draft.
  function patchProfile(next: Partial<Profile>) {
    onSave({ ...profile, ...next });
  }

  function patchTp(next: Partial<TaskDraft>) {
    onSave({ ...profile, taskPage: { ...tp, ...next } });
  }

  function patchWidget(kind: PageWidget["kind"], next: Partial<PageWidget>) {
    patchTp({ widgets: tp.widgets.map((w) => (w.kind === kind ? { ...w, ...next } : w)) });
  }

  function moveWidget(index: number, dir: -1 | 1) {
    const to = index + dir;
    if (to < 0 || to >= tp.widgets.length) return;
    const next = tp.widgets.slice();
    [next[index], next[to]] = [next[to], next[index]];
    patchTp({ widgets: next });
  }
  ...
  function updatePreset(i: number, value: number) {
    const next = tp.presets.slice();
    next[i] = Math.max(1, Math.round(value) || 1);
    patchTp({ presets: next });
  }

  function addPreset() {
    const last = tp.presets[tp.presets.length - 1] ?? 5;
    patchTp({ presets: [...tp.presets, last + 5] });
  }

  function removePreset(i: number) {
    if (tp.presets.length <= 1) return;
    patchTp({ presets: tp.presets.filter((_, j) => j !== i) });
  }
  ...
  function onBackgroundFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => patchTp({ design: { background: { type: "image", value: String(reader.result) } } });
    reader.readAsDataURL(file);
  }
    ...
    <div className={styles.wrap}>
      <div className={styles.builder}>
        <div className={styles.editor}>
          <div className={styles.tabs} role="tablist" aria-label="Task builder">
                ...
                <div className={styles.bioField}>
                  ...
                  <div className={styles.bioBox}>
                    <textarea
                      ...
                      rows={2}
                      maxLength={TASK_HEADLINE_MAX}
                      placeholder="e.g. Set me a dare — money back if I chicken out"
                      value={tp.headline}
                      onChange={(e) => patchTp({ headline: e.target.value })}
                    />
                    <span className={styles.charCount}>{tp.headline.length}/{TASK_HEADLINE_MAX}</span>
                  </div>
                  <div className="footnote">This is the pitch on your task page. The escrow rules themselves live in Settings.</div>
                </div>

                <div className={styles.bioField}>
                  <label className={`toggle${tp.descriptionEnabled ? " on" : ""}`}>
                    <span className="track"><span className="knob" /></span>
                    <input type="checkbox" hidden checked={tp.descriptionEnabled} onChange={(e) => patchTp({ descriptionEnabled: e.target.checked })} />
                    Description
                  </label>
                  <div className={styles.bioBox}>
                    <textarea
                      ...
                      placeholder="What you will and won't do — the limits, so nobody wastes money"
                      value={tp.description}
                      onChange={(e) => patchTp({ description: e.target.value })}
                    />
                    <span className={styles.charCount}>{tp.description.length}/{TASK_DESCRIPTION_MAX}</span>
                  </div>
                </div>
              </div>
                    ...
                    <div key={w.kind} className={styles.widgetGroup}>
                      <div className={`${styles.widgetRow}${w.enabled ? "" : ` ${styles.widgetOff}`}`}>
                        ...
                        <div className={styles.widgetStepper}>
                          ...
                          <button type="button" aria-label="Move down" disabled={i === tp.widgets.length - 1} onClick={() => moveWidget(i, 1)}>▼</button>
                        </div>
                        <button type="button" className={styles.widgetNameBtn} aria-expanded={open} onClick={() => toggleWidgetConfig(w.kind)}>
                          {TASK_WIDGET_LABEL[w.kind]}
                      ...
                      {open && w.kind === "donate" && (
                        <div className={styles.widgetConfig}>
                          <div className={styles.presetRow}>
                            ...
                            {tp.presets.length < MAX_TASK_PRESETS && (
            ...
            <div className={styles.tabBody}>
              <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div className={styles.themeGrid}>
                  {THEMES.map((t) => {
                    const active = sameBackground(tp.design, t.design);
                    return (
                      <button
                        key={t.id}
                        type="button"
                        ...
                        onClick={() => patchTp({ design: t.design })}
                      >
              ...
              <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div className={styles.bgTypeRow}>
                  {(["color", "gradient", "image"] as const).map((t) => (
                    <button
                      key={t}
                      type="button"
                      className={`${styles.bgType} ${tp.design.background.type === t ? styles.bgTypeOn : ""}`}
                      onClick={() => {
                        if (t === "image") { bgFileRef.current?.click(); return; }
                        patchTp({ design: { background: { type: t, value: t === "color" ? BACKGROUND_COLOR_PRESETS[0] : BACKGROUND_GRADIENT_PRESETS[0].id } } });
                      }}
                    >
                ...
                {tp.design.background.type === "color" && (
                  <div className={styles.swatches}>
                    {BACKGROUND_COLOR_PRESETS.map((hex) => (
                      <button
                        ...
                        type="button"
                        aria-label={hex}
                        className={`${styles.swatch} ${tp.design.background.value === hex ? styles.swatchOn : ""}`}
                        style={{ background: hex }}
                        onClick={() => patchTp({ design: { background: { type: "color", value: hex } } })}
                      />
                    ))}
                    <input
                      className={styles.hexInput}
                      type="text"
                      value={tp.design.background.value}
                      onChange={(e) => patchTp({ design: { background: { type: "color", value: e.target.value } } })}
                    />
                  </div>
                )}

                {tp.design.background.type === "gradient" && (
                  <div className={styles.swatches}>
                    {BACKGROUND_GRADIENT_PRESETS.map((g) => (
                      <button
                        ...
                        type="button"
                        aria-label={g.label}
                        className={`${styles.swatch} ${tp.design.background.value === g.id ? styles.swatchOn : ""}`}
                        style={{ backgroundImage: g.css }}
                        onClick={() => patchTp({ design: { background: { type: "gradient", value: g.id } } })}
                      />
                    ))}
                  </div>
                )}

                {tp.design.background.type === "image" && (
                  <div className="footnote">
                    {tp.design.background.value ? "Image set — pick Choose file to replace it." : "No image chosen yet."}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className={styles.previewCol}>
          ...
          <LivePreview src={`/@${profile.handle}/task`} device={device} />

          <div className={styles.linkRow}>
            <div className={styles.linkLabel}>Task link</div>
            <a className={styles.linkChip} href={`/@${profile.handle}/task`} target="_blank" rel="noreferrer">
              ...
              <div className={styles.qrPop} role="dialog" aria-label="QR code">
                {qrDataUrl ? (
                  <>
                    ...
                    <a className={styles.qrDownload} href={qrDataUrl} download="crown-task-qr.png">
                      Download PNG
                    </a>
                  </>
                ) : (
